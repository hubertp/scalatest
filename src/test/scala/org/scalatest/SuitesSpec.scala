...
class SuitesSpec extends FunSpec with SharedHelpers {
  ...
  describe("Suites") {
    ...
    it("should care about chosenStyles if it contains tests directly") {
      ...
      g.run(None, SilentReporter, new Stopper {}, Filter(), Map("org.scalatest.ChosenStyles" -> Set("Suite")), None, new Tracker)
      intercept[NotAllowedException] {
        // Should not allow if chosen styles is FunSuite, because Suite is the style of *tests* written in this Suites
        g.run(None, SilentReporter, new Stopper {}, Filter(), Map("org.scalatest.ChosenStyles" -> Set("FunSuite")), None, new Tracker)
      }
    }
  }
