...
class RunNotifierSuite extends FunSuite {
  ...
  test("report(TestStarting) generates a fireTestStarted invocation") {
    ...
    reporter(TestStarting(new Ordinal(99), "suite name", "suite ID", Some("suite.class.name"), None, "some test name", "some test name", None))
    ...
    reporter(TestStarting(new Ordinal(99), "suiteName", "suite ID", Some("suite.class.name"), None, "someTestName", "someTestName", None))
    ...
    reporter(TestStarting(ordinal, "suiteName", "suite ID", None, None, "theTestName", "theTestName", None))
  ...
  test("report(TestFailed) generates a fireTestFailure invocation") {
    ...
    reporter(TestFailed(ordinal, "No msg", "SuiteClassName", "suite ID", Some("fully.qualified.SuiteClassName"), None, "theTestName", "theTestName", None, List.empty[Event], Some(exception)))
    assert(runNotifier.passed.get.getDescription.getDisplayName === "theTestName(fully.qualified.SuiteClassName)")
    reporter(TestFailed(ordinal, "No msg", "SuiteClassName", "suite ID", None, None, "theTestName", "theTestName", None, List.empty[Event], Some(exception)))
  ...
  test("report(TestSucceeded) generates a fireTestFinished invocation") {
    ...
    val reporter = new RunNotifierReporter(runNotifier)
    reporter(TestSucceeded(ordinal, "SuiteClassName", "suite ID", Some("fully.qualified.SuiteClassName"), None, "theTestName", "theTestName", None, List.empty[Event]))
    assert(runNotifier.passed.get.getDisplayName === "theTestName(fully.qualified.SuiteClassName)")
    reporter(TestSucceeded(ordinal, "SuiteClassName", "suite ID", None, None, "theTestName", "theTestName", None, List.empty[Event]))
  ...
  test("report(TestIgnored) generates a fireTestIgnored invocation") {
    ...
    val reporter = new RunNotifierReporter(runNotifier)
    reporter(TestIgnored(new Ordinal(99), "suite name", "suite ID", Some("suite class name"), None, "some test name", "some test name", None))
    assert(runNotifier.methodInvocationCount === 1)
    assert(runNotifier.passed.get.getDisplayName === "some test name(suite class name)")

    reporter(TestIgnored(new Ordinal(99), "suiteName2", "suite ID", Some("suite.class.name"), None, "testName", "testName", None))
    assert(runNotifier.methodInvocationCount === 2)
    assert(runNotifier.passed.get.getDisplayName === "testName(suite.class.name)")

    reporter(TestIgnored(ordinal, "SuiteClassName", "suite ID", Some("fully.qualified.SuiteClassName"), None, "theTestName", "theTestName", None))
    assert(runNotifier.passed.get.getDisplayName === "theTestName(fully.qualified.SuiteClassName)")
    reporter(TestIgnored(ordinal, "SuiteClassName", "suite ID", None, None, "theTestName", "theTestName", None))
    assert(runNotifier.passed.get.getDisplayName === "theTestName(SuiteClassName)")
  }
  ...
  test("report(SuiteAborted) generates a fireTestFailure invocation") {
    ...
    reporter(SuiteAborted(ordinal, "some message", "SuiteClassName", "suite ID", Some("fully.qualified.SuiteClassName"), None, Some(exception)))
    assert(runNotifier.methodInvocationCount === 1)
    ...
    reporter(SuiteAborted(ordinal, "a different message", "SuiteClassName", "suite ID", Some("fully.qualified.SuiteClassName"), None, Some(otherException)))
    assert(runNotifier.methodInvocationCount === 2)
    assert(runNotifier.passed.get.getException === otherException)
    assert(runNotifier.passed.get.getDescription.getDisplayName === "fully.qualified.SuiteClassName")

    reporter(SuiteAborted(ordinal, "No msg", "SuiteClassName", "suite ID", Some("fully.qualified.SuiteClassName"), None, Some(exception)))
    assert(runNotifier.passed.get.getDescription.getDisplayName === "fully.qualified.SuiteClassName")
    reporter(SuiteAborted(ordinal, "No msg", "SuiteClassName", "suite ID", None, None, Some(exception)))
