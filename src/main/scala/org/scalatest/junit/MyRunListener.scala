...
import exceptions._
  ...
  extends RunListener {
    ...
    override def testFailure(failure: Failure) {
      ...
      report(TestFailed(theTracker.nextOrdinal(), message, testClassName, testClass, Some(testClass), getDecodedName(testClassName), testName, testName, getDecodedName(testName), List.empty[Event], throwable, None, Some(formatter), Some(SeeStackDepthException), None, payload))
    ...
    override def testFinished(description: Description) {
      if (!failedTests.contains(description.getDisplayName)) {
        ...
        val formatter = getIndentedText(testName, 1, true)
        report(TestSucceeded(theTracker.nextOrdinal(), testClassName, testClass, Some(testClass), getDecodedName(testClassName), testName, testName, getDecodedName(testName), List.empty[Event], None, Some(formatter), getTopOfMethod(testClass, testName)))
        // TODO: can I add a duration?
      }
    }
