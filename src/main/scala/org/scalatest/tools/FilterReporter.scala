...
private[tools] class FilterReporter(reporter: Reporter, configSet: Set[ReporterConfigParam]) extends ResourcefulReporter {
  ...
  override def apply(event: Event) {
    ...
    event match {
      ...
      case event: TestFailed => 
          report(event.copy(testEvents = filterInfoMarkupProvided(event.testEvents, configSet.contains(FilterInfoProvided), configSet.contains(FilterMarkupProvided))))
  ...
  private def filterInfoMarkupProvided(testEvents: Seq[Event], filterInfoProvided: Boolean, filterMarkupProvided: Boolean): Seq[Event] = {
    testEvents.filter { e =>
      e match {
        case infoProvided: InfoProvided => !filterInfoProvided
        case markupProvided: MarkupProvided => !filterMarkupProvided
        case _ => true
      }
    }
  }
