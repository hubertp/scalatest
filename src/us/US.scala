...
trait StateSuite extends Suite {
 ...
 private def reportTestFailed(theSuite: Suite, report: Reporter, throwable: Throwable, testName: String, testText: String, decodedTestName: Option[String],
    ...
    report(TestFailed(tracker.nextOrdinal(), message, theSuite.suiteName, theSuite.suiteId, Some(theSuite.getClass.getName), getDecodedName(theSuite.suiteName), testName, testText, decodedTestName, List.empty[Event], Some(throwable), Some(duration), Some(formatter), location, suiteRerunner))
  }
  ...
  private def reportTestCanceled(theSuite: Suite, report: Reporter, throwable: Throwable, testName: String, testText: String,
    ...
    val message = getMessageForException(throwable)
    val formatter = getIndentedText(testText, level, includeIcon)
    report(TestCanceled(tracker.nextOrdinal(), message, theSuite.suiteName, theSuite.suiteId, Some(theSuite.getClass.getName), getDecodedName(theSuite.suiteName), testName, testText, getDecodedName(testName), List.empty[Event], Some(throwable), Some(duration), Some(formatter), location, rerunnable))
  }
  ...
  private def reportTestSucceeded(theSuite: Suite, report: Reporter, tracker: Tracker, testName: String, testText: String, decodedTestName: Option[String], duration: Long, formatter: Formatter, suiteRerunner: Option[String], location: Option[Location]) {
    report(TestSucceeded(tracker.nextOrdinal(), theSuite.suiteName, theSuite.suiteId, Some(theSuite.getClass.getName), getDecodedName(theSuite.suiteName), testName, testText, decodedTestName, List.empty[Event], Some(duration), Some(formatter),
      location, suiteRerunner))
  }
  ...
  private def reportTestPending(theSuite: Suite, report: Reporter, tracker: Tracker, testName: String, testText: String, decodedTestName: Option[String], duration: Long, formatter: Formatter, location: Option[Location]) {
    report(TestPending(tracker.nextOrdinal(), theSuite.suiteName, theSuite.suiteId, Some(theSuite.getClass.getName), getDecodedName(theSuite.suiteName), testName, testText, decodedTestName, List.empty[Event], Some(duration), Some(formatter),
      location))
  }
  ...
  private def handleFailedTest(throwable: Throwable, testName: String, decodedTestName: Option[String],
    ...
    report(TestFailed(tracker.nextOrdinal(), message, suiteName, suiteId, getDecodedName(suiteName), Some(getClass.getName), testName, testName, decodedTestName, List.empty[Event], Some(throwable), Some(duration), Some(formatter), location, None))
  }
    ...
    testStatuses(simpleName)(testName) match {
      ...
      case Canceled(duration, remaining) =>
        if (remaining > 1)
          ...
          val formatter = getIndentedText(testName, 1, true)
          args.reporter(TestCanceled(args.tracker.nextOrdinal(), message, suiteName, suiteId, getDecodedName(suiteName), Some(getClass.getName), testName, testName, getDecodedName(testName), List.empty[Event], Some(e), Some(duration), Some(formatter), Some(ToDoLocation), None))
